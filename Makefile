...
all:
	...
	GOOS=linux GOARCH=amd64 go build -tags shell,portforwards,passauth -ldflags "-s -w -X main.bindPortStr=${BINDPORT}"
	GOOS=windows GOARCH=386 go build -tags shell,portforwards,passauth -ldflags "-s -w -X main.bindPortStr=${BINDPORT}" -o bindshell.exe .

shellonly:
	#GOOS=linux GOARCH=amd64 go build -tags shell,certauth,pubkeyauth,passauth -ldflags "-s -w -X main.bindPortStr=${BINDPORT}"
	#GOOS=windows GOARCH=386 go build -tags shell,certauth,pubkeyauth,passauth -ldflags "-s -w -X main.bindPortStr=${BINDPORT}" -o bindshell.exe .
	GOOS=linux GOARCH=amd64 go build -tags shell,passauth -ldflags "-s -w -X main.bindPortStr=${BINDPORT}"
