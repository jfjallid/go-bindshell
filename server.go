...
func NewServer(bindAddr string) {
    ...
    if _, ok := features["password"]; ok {
        config.PasswordCallback = func(c ssh.ConnMetadata, pass []byte) (*ssh.Permissions, error) {
            ...
            //allowedPsk := sha256.Sum256([]byte("tiger"))

			// Should use constant-time compare in a production setting.
            // Take a look at crypto/subtle constant time compares
